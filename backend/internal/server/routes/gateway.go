...
func RegisterGatewayRoutes(
	...
	var conversationRecorder securityaudit.ConversationRecorder
	if h != nil && h.Admin != nil && h.Admin.PromptAudit != nil {
		conversationRecorder = h.Admin.PromptAudit.ConversationRecorder()
	}
	conversationArchive := handler.ConversationArchiveMiddleware(conversationRecorder)
	...
	gateway.Use(conversationArchive)
	...
	gemini.Use(conversationArchive)
	...
	r.POST("/responses", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), conversationArchive, compositeTarget, requireGroupAnthropic, responsesHandler)
	r.POST("/responses/*subpath", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), conversationArchive, compositeTarget, requireGroupAnthropic, guardResponsesSubpath(responsesHandler))
	...
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), conversationArchive, compositeTarget, requireGroupAnthropic)
	{
	...
	r.POST("/chat/completions", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), conversationArchive, compositeTarget, requireGroupAnthropic, func(c *gin.Context) {
		if isOpenAIResponsesCompatibleGatewayPlatform(c) {
			h.OpenAIGateway.ChatCompletions(c)
			return
		}
	...
	antigravityV1.Use(conversationArchive)
	...
	antigravityV1Beta.Use(conversationArchive)
