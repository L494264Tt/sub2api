...
func (s *UserRepoSuite) TestBatchUpdateLimitsUpdatesOnlyProvidedFields() {
	...
	affected, err := s.repo.BatchUpdateLimits(s.ctx, []int64{user.ID}, &concurrency, nil, nil, nil, nil, false)
	s.Require().NoError(err)
	s.Equal(1, affected)
...
func (s *UserRepoSuite) TestBatchUpdateLimitsUpdatesBothFieldsToZero() {
	...
	affected, err := s.repo.BatchUpdateLimits(s.ctx, []int64{user.ID}, &zero, &zero, nil, nil, nil, false)
	s.Require().NoError(err)
	s.Equal(1, affected)
...
func (s *UserRepoSuite) TestBatchUpdateLimitsIgnoresDeletedUsersAndReturnsAffectedRows() {
	...
	affected, err := s.repo.BatchUpdateLimits(s.ctx, []int64{active.ID, deleted.ID}, nil, &rpmLimit, nil, nil, nil, false)
	s.Require().NoError(err)
	s.Equal(1, affected)
